...
export function CalendarPage() {
  ...
  // Responsive Week View State (adjusts columns based on screen size)
  const [daysToShow, setDaysToShow] = useState(7);

  // --- Effects ---
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) setDaysToShow(1); // Mobile
      else if (window.innerWidth < 1024) setDaysToShow(3); // Tablet
      else setDaysToShow(7); // Desktop
    };
  ...
  const next = () => {
    ...
    else setCurrentDate(addDays(currentDate, daysToShow));
  };
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden direction-rtl">
          ...
          <div className="space-y-2">
            {events
              ...
              .map(e => (
                ...
                >
                  <div className={`w-2 h-2 rounded-full ${e.color ? JSON.parse(e.color).bg.replace('bg-', 'bg-') : 'bg-primary'}`} />
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Header */}
        <header className="flex items-center justify-between p-4 border-b border-border/40 bg-background/80 backdrop-blur-xl z-10">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-black tracking-tight min-w-[140px]">
              {view === "day" 
                ? format(currentDate, "d MMMM yyyy", { locale: arSA })
                : format(currentDate, "MMMM yyyy", { locale: arSA })}
            </h2>
            <button onClick={today} className="text-sm font-black text-primary hover:bg-primary/10 px-4 py-2 rounded-full transition-all active:scale-[0.97]">
              اليوم
            </button>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex bg-secondary/50 p-1 rounded-2xl text-xs font-black uppercase tracking-widest">
              <button 
                onClick={() => setView("month")}
                className={cn("px-4 py-1.5 rounded-xl transition-all", view === "month" ? "bg-card shadow-sm text-primary" : "text-muted-foreground hover:text-foreground")}
              >
                شهر
              </button>
              <button 
                onClick={() => setView("week")}
                className={cn("px-4 py-1.5 rounded-xl transition-all", view === "week" ? "bg-card shadow-sm text-primary" : "text-muted-foreground hover:text-foreground")}
              >
                أسبوع
              </button>
              <button 
                onClick={() => setView("day")}
                className={cn("px-4 py-1.5 rounded-xl transition-all", view === "day" ? "bg-card shadow-sm text-primary" : "text-muted-foreground hover:text-foreground")}
              >
                يوم
              </button>
            </div>
            <button 
              ...
              className="ios-button-primary h-10 px-5 rounded-full text-sm font-black flex items-center justify-center gap-2 shadow-lg shadow-primary/20"
            >
              <Plus size={20} />
              <span className="hidden sm:inline">حدث جديد</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-hidden relative">
          {view === "week" || view === "day" ? (
            <WeekView 
              currentDate={currentDate} 
              events={events} 
              daysToShow={view === "day" ? 1 : daysToShow}
...
function WeekView({ currentDate, events, daysToShow, onEventClick, onSlotClick, onEventUpdate, onDayClick }: any) {
  // Calculate start date based on daysToShow
  // If 7 days, start from Sunday. If < 7, start from currentDate.
  const start = daysToShow === 7 
        ...
        {days.map(day => {
          ...
          return (
            <div key={day.toISOString()} className={cn(
              ...
              <div className={cn("text-[11px] font-bold uppercase tracking-wider mb-1.5", isToday(day) ? "text-primary" : "text-muted-foreground")}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto relative scroll-smooth">
        ...
        <div className="flex relative min-h-[1440px]">
          ...
          <div className="w-14 flex-shrink-0 border-l border-border/60 bg-background/95 backdrop-blur sticky right-0 z-20">
            {HOURS.map(hour => (
              <div key={hour} className="h-[60px] text-[10px] font-medium text-muted-foreground text-center relative -top-2.5">
                ...
                {dayEvents.map((e: CalendarEvent) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="opacity-70 truncate text-[10px] font-medium">
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 border-b border-border">
        {WEEK_DAYS.map(d => (
          <div key={d} className="py-2 text-center text-sm font-medium text-muted-foreground border-l border-border last:border-l-0">
            {d}
          </div>
        ))}
      </div>
            ...
            >
              <div className="flex justify-center mb-1">
                <button 
                  ...
                  isToday(day) ? "bg-primary text-primary-foreground hover:bg-primary/90" : "text-muted-foreground"
                )}>
                  {format(day, "d")}
                </button>
              </div>
                ...
                {dayEvents.slice(0, 3).map((e: CalendarEvent) => {
                   ...
                   return (
                    <div 
                      ...
                      onClick={(ev) => { ev.stopPropagation(); onEventClick(e); }}
                      className={cn(
                        "text-[10px] px-1.5 py-0.5 rounded truncate font-medium border cursor-pointer",
                        colorStyle.bg, colorStyle.text, colorStyle.border
                      )}
                    >
                      {e.title}
...
function CurrentTimeIndicator({ start }: { start: Date }) {
  ...
  if (!isWithinInterval(now, { start, end: addDays(start, 7) })) return null;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 relative">
         ...
         <div 
           className="absolute w-2.5 h-2.5 bg-red-500 rounded-full -mt-[5px] shadow-[0_0_8px_rgba(239,68,68,0.6)]"
           style={{ right: `${(dayIndex / 7) * 100}%`, transform: 'translateX(50%)' }} // RTL adjustment
         >
            ...
            <div className="bg-card rounded-2xl overflow-hidden border border-border/50 divide-y divide-border/40">
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <select 
                  ...
                  className="bg-transparent text-muted-foreground outline-none text-sm font-medium cursor-pointer appearance-none text-left"
                  dir="ltr"
                >
